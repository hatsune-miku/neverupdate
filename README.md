...
**NeverUpdate (NU) 是一个 Windows 11 平台专用的系统更新阻断工具，旨在将用户被剥夺的“不更新系统”的权利归还给用户。**

它通过多层机制（服务注册表篡改、hosts/防火墙规则、组策略、计划任务 XML 补丁、兜底注册表设置）持续**对抗 Windows Update 的自动下载与安装**，并提供一个 Tauri 桌面 GUI 和一个以 Windows 服务形式运行的守护进程来保障阻断状态的持久性。
