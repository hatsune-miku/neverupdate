...
TLDR: 真正长期有效禁止 Windows 自动更新，同时保住微软商店。

# 安全性声明

1. NeverUpdate 会自我提权为 `TrustedInstaller`，并进行破坏系统组件的操作。使用 NeverUpdate 则代表您知晓对系统组件引入破坏性操作的风险，并愿意承担由此带来的全部后果（包括短期影响和长期潜在影响），且同意如果出现任何问题，均与开发者无关。

2. 并非所有的更新都是无用和有风险的——当你拒绝更新系统时，你也一并拒绝了**重要的安全更新和有趣的新功能**。务必知晓这些风险。由于未及时安装安全补丁导致的任何后果，也与开发者无关。

> NeverUpdate 是一个“用户侧系统否决器（User-side System Veto Tool）”，
> 用于在平台不可控时，强制恢复用户对更新行为的最终决定权。

## 概述

**NeverUpdate (NU) 是一个 Windows 11 平台专用的系统更新阻断工具，旨在简易、彻底、长期可靠、可逆地停止 Windows 自动更新，将用户被剥夺的“不更新系统”的权利归还给用户。**
...
> “被自动推送了 BIOS 更新，更新到了一个带有 bug 的版本，**一夜之间好好的电脑突然就开机卡自检了**，定位问题、修电脑、丢失数据（！）所花的代价，就是开发 NU 的无数动力之一。”

Windows 的“事实强制”的更新策略，曾经坑惨过无数用户。Windows Update 团队显然没有意识到 BIOS 固件这种东西是什么概念，对普通人而言，这绝非一个应该频繁更新的东西。

事实上，很多上层的驱动软件也是如此——很多用户有他们固定使用的稳定版本，不希望更新。但 Windows 更新仍然默认开启此类更新的推送。

是时候从 Windows 更新手中**撤回**系统的控制权了。
